-- 057_add_daily_breakdown_and_usd_kpis.sql
-- Enhances get_monthly_summary with:
--   1. New monthly KPI fields:
--        usdt_net        – net USDT = usdt_deposits - usdt_withdrawals (can be negative)
--        commission_usd  – total commission expressed in USD
--        bank_usd_gross  – non-USDT transfers total in USD (via stored amount_usd)
--        usd_cevirim     – bank_usd_gross + usdt_net  ("USD ÇEVRİM" in the spreadsheet)
--        kom_son_usd     – usd_cevirim - commission_usd ("KOM. SON USD")
--        finans_pct      – commission_usd / usd_cevirim * 100 ("FİNANS %")
--   2. New return key: daily_detailed
--        Per-day breakdown: day, bank_try, kk_try, commission_try,
--        usdt_net, avg_rate, bank_usd, commission_usd,
--        usd_cevirim, kom_son_usd, finans_pct
...
BEGIN
  ...
  kpis_base AS (
    SELECT
      ...
      -- Commission in TRY (USDT commissions converted via exchange_rate)
      coalesce(sum(CASE WHEN currency = 'USD'
                        THEN commission * coalesce(exchange_rate, 1)
                        ELSE commission END), 0)                                                     AS total_commission_try,
      -- Counts
      ...
      -- Commission expressed in USD
      --   USDT transfers: commission column is already in USD
      --   TRY transfers : commission column is in TRY, divide by exchange_rate
      coalesce(sum(CASE WHEN currency = 'USD'
                        THEN commission
                        ELSE commission / NULLIF(coalesce(exchange_rate, 1), 0)
                   END), 0)                                                                          AS commission_usd,
  ...
  daily_details_raw AS (
    SELECT
      ...
      coalesce(sum(CASE WHEN currency = 'USD'
                        THEN commission * coalesce(exchange_rate, 1)
                        ELSE commission END), 0)                                         AS commission_try,
  ...
  -- ── Breakdown aggregates (unchanged) ─────────────────────────────────────
