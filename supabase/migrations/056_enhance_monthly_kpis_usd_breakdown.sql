-- 056_enhance_monthly_kpis_usd_breakdown.sql
-- Add per-direction USDT and Bank+CC USD breakdowns to get_monthly_summary KPIs.
-- New fields in kpis and prev_kpis:
--   usdt_deposits_usd       – USDT deposits (USD face value)
--   usdt_withdrawals_usd    – USDT withdrawals (USD face value)
--   bank_cc_deposits_usd    – Bank+CC deposits converted to USD via stored exchange rate
--   bank_cc_withdrawals_usd – Bank+CC withdrawals converted to USD
...
BEGIN
  ...
  kpis AS (
    SELECT
      ...
      -- Per-direction USDT breakdown (USD face value, currency = 'USD')
      ...
      -- Per-direction Bank+CC breakdown (non-USDT transfers expressed in USD via amount_usd)
